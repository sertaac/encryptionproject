...
class PasswordProtectionDetector:
    ...
    async def analyze_file(self, file_path: str) -> Dict: # Ensure this is async def
        """
        ...
        try:
            if file_type in self.handlers:
                ...
                # is_encrypted is now async, so await directly
        ...
        if confidence < 0.5:
            # EntropyAnalyzer.analyze is a blocking method, run in executor
