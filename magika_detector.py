...
class MagikaDetector:
    ...
    async def detect(self, file_path: str) -> str: # Change to async def
        ...
        """
        try:
            # Get Magika's prediction - Run in a separate thread as it's blocking #
            result = await asyncio.to_thread(self.model.identify_path, file_path) # Wrap with asyncio.to_thread
